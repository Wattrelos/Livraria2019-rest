...
var pagination = function (page, taglabe2){
    ...
    $.ajax({				
        ...
        error: function (erro, textStatus, xhr) {
        	console.log(erro);        	
        	alert("Erro: "+ erro.status + status + "Falha ao tentar acessar "+ taglabe3 + "!");
        }
	});
	...
	// PedidoPaginar testes antes do encapsulamento
	var paginationPedidos = function (page, taglabe2){
		var taglabe3 = taglabe2.toLowerCase();
		var url = "http://localhost:8080/" + taglabe3 + "/page?page=" + page;
			// Listar dados-----------------------------------------
			$.getJSON(url, function (result){
				...
				result.content.forEach(function(field, index){
					_text +=
				   `<div>${field.observacao}</div>
					<div>${field.dataCadastro.slice(0, 10)}</div>
					<div><button type="submit" onclick="confirmUpdate('${taglabe3}','${field.id}','${field.observacao}')">Editar</button></div>
					<div><button type="submit" onclick="confirmDelete('${taglabe3}','${field.id}','${field.observacao}')">Excluir</button></div>`;
				});
				_text += `</div>
				<form method="post" enctype="application/json" action="/${taglabe3}" onsubmit="confirmCreate(this); return false;">
					<input type="text" id="nome" name="nome" class="form-data" placeholder="${taglabe3}">
